...
def compute_score(my_trees, my_flowers, my_bushes, my_houseplants):
	tree_scores = [count[x] * CO2_absorption[x] * O2_production[x] / rarity[x] for x in my_trees]
	flower_scores = [count[x] * CO2_absorption[x] * O2_production[x] / rarity[x] for x in my_flowers]
	bush_scores = [count[x] * CO2_absorption[x] * O2_production[x] / rarity[x] for x in my_bushes]
	houseplant_scores = [count[x] * CO2_absorption[x] * O2_production[x] / rarity[x] for x in my_houseplants]
	...
	all_scores_sum = tree_scores_sum + flower_scores_sum + bush_scores_sum + houseplant_scores_sum
	...
	frequency[3] = houseplant_scores_sum * (1 - (houseplant_scores_sum / all_scores_sum))
